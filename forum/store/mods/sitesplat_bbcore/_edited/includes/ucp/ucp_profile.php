				...
				if ($submit)
				{
					...
					if (!sizeof($error))
					{
						$sql_ary = array(
							...
							'user_avatar'		=> ($auth->acl_get('u_chgavatar') && $user->data['user_avatar_type'] == 9) ? 'http://www.gravatar.com/avatar/' . md5(strtolower(trim((($auth->acl_get('u_chgemail')) ? $data['email'] : $user->data['user_email'])))) . '?d=identicon&s=' . min($config['avatar_max_width'], $config['avatar_max_height']) : $user->data['user_avatar'],
						);
