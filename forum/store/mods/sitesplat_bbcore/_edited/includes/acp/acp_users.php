					...
					if (!sizeof($error))
					{
						...
						if ($update_email !== false)
						{
							$sql_ary += array(
								...
								'user_avatar'		=> ($user_row['user_avatar_type'] == 9) ? 'http://www.gravatar.com/avatar/' . md5(strtolower(trim($update_email))) . '?d=identicon&s=' . min($config['avatar_max_width'], $config['avatar_max_height']) : $user_row['user_avatar'],
							);
